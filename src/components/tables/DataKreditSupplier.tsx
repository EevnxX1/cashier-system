      ...
      <div className="flex w-full flex-col">
        ...
        {order.items.map((item) => (
          ...
          >
            ...
            <div className="mb-3 flex w-full items-center justify-between">
              <h2>Total Harga</h2>
              <p>Rp. {formatRupiah(item.subtotal)}</p>
            </div>
          </section>
        ))}
      </div>
...
const handleDetail = (order: Order) => {
  ...
  const fetchPembayaran = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/pembayaran-hutang`,
      {
        method: "GET",
        headers: {
...
const handleBayar = async (order: Order) => {
        ...
        <div className="flex justify-end gap-2">
          <Button
            ...
            onClick={async () => {
              ...
              console.log("kode_supplier", order.kode_supplier);
              ...
              const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/api/pembayaran-hutang`,
                {
                  ...
                  body: JSON.stringify({
                    kode_supplier: order.kode_supplier,
                    tanggal: tanggal,
  ...
  if (result) {
    ...
    Swal.fire({
      icon: "success",
      title: "Pembayaran Hutang Supplier Berhasil Dilakukan",
      background: "#23272a",
      color: "#fff",
      timer: 2000,
      showConfirmButton: false,
...
export default function DataKreditSupplier() {
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {currentItems.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                      {order.supplier.nama_supplier == null
                        ? "Supplier Tidak Ditemukan"
                        : order.supplier.nama_supplier}
                    </TableCell>
                    <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                      {order.tanggal == null ? "Cash" : order.tanggal}
                    </TableCell>
                    <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                      {order.tempo === null ? "Cash" : order.tempo}
                    </TableCell>
                    <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
