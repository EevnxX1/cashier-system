...
export default function DataPembelianKredit({
  ...
  const handleCreate = async () => {
          ...
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/produk`, {
            ...
            .then((data) => {
              ...
              if (selectedData) {
                ...
                setHarga(selectedData.harga_customer);
                console.log("stok:", selectedData.stok);
                console.log("harga:", selectedData.harga_customer);
              }
            });
        }
    ...
    await MySwal.fire({
      title: "Create Data Pembelian Kredit",
      html: <CreateForm />,
    ...
    <div className="flex flex-col">
      <div
        className={clsx("mb-5 w-full text-white", {
          flex: tanggal && namaCustomer,
          hidden: !tanggal || !namaCustomer,
        })}
      >
        <div className="flex w-full flex-col gap-y-5">
          ...
          <div className="flex w-full justify-between">
            <h3>Nama Customer</h3>
            <div className="w-[70%]">
              <p className="text-start">: {namaCustomer}</p>
            </div>
          </div>
          <div className="flex w-full justify-between">
            <h3>Termin</h3>
                ...
                {pembelianItems.items.map((order, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-theme-sm px-4 py-3 text-start text-gray-500 dark:text-gray-400">
                      Rp.{" "}
                      {
                        // menampilkan harga produk dari kode produk
                        formatRupiah(
                          String(
                            produk.find(
                              (item) => item.kode_produk === order.kode_produk,
                    ...
                    item.qty *
                      Number(
                        produk.find(
                          (item) =>
                            item.kode_produk ===
                            pembelianItems.items[index].kode_produk,
                        )?.harga_customer,
                      ),
                  0,
                ),
